...
import { isMobileBrowser, isElectron } from "../lib/platform";
...
// On mobile always use browser cam; hide bridge + server options
const AVAILABLE_MODES: CaptureMode[] = _isMobile
  ? ["browser"]
  : ["bridge", "browser", "server"];

const DEFAULT_MODE: CaptureMode = _isMobile ? "browser" : "bridge";
...
export function CameraView() {
  ...
  useEffect(() => {
    // On desktop: switch to bridge if WSL detected
    if (!_isMobile && cam.wsl) setCaptureMode("bridge");
  }, [cam.wsl]);
  ...
  const showWslHint = !_isMobile && Boolean(health.wsl || cam.wsl) && captureMode !== "bridge";
      ...
      <div className="camera-main card">
        <div className="card-head">
          <div>
            ...
            <p className="meta">
              {_isMobile
                ? "Phone camera → TriadNet → intent prediction"
                : "Webcam → TriadNet → speak intent when confidence is high"}
            </p>
          </div>
          <div className="status-chips">
            ...
            {!_isMobile && (health.wsl || cam.wsl) ? <span className="chip chip-info">WSL</span> : null}
        ...
        {/* Only show mode tabs on desktop / Electron */}
        {!_isMobile ? (
          <div className="mode-tabs" role="tablist">
            {(AVAILABLE_MODES as CaptureMode[]).map((m) => (
              <button
                key={m}
                type="button"
                role="tab"
        ...
        <div className="video-stage">
          ...
          {cam.status === "idle" ? (
            <div className="video-placeholder">
              ...
              <p className="meta">
                {_isMobile
                  ? "Tap Start to use your phone camera."
                  : captureMode === "bridge"
